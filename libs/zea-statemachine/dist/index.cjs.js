...
class State {
   ...
   */
  constructor(name) {
    this.__name = name ? name : zeaEngine.Registry.getBlueprintName(this);
  ...
  toJSON(context, flags) {
    const j = {
      name: this.__name,
      type: zeaEngine.Registry.getBlueprintName(this),
    };
  ...
  fromJSON(j, context, flags) {
    ...
    for (const stateEventJson of j.stateEvents) {
      const stateEvent = zeaEngine.Registry.constructClass(stateEventJson.type);
      stateEvent.fromJSON(stateEventJson, context);
      this.addStateEvent(stateEvent);
    }
    for (const activationActionJson of j.activationActions) {
      const activationAction = zeaEngine.Registry.constructClass(activationActionJson.type);
    ...
    for (const deactivationActionJson of j.deactivationActions) {
      const deactivationAction = zeaEngine.Registry.constructClass(deactivationActionJson.type);
...
zeaEngine.Registry.register('State', State);
...
class StateAction extends zeaEngine.ParameterOwner {
   ...
   */
  activate() {
    console.warn('activate must be implmented by each action. this:' + zeaEngine.Registry.getBlueprintName(this));
  }
  ...
  fromJSON(j, context, flags) {
    ...
    for (const childActionjson of j.childActions) {
      const childAction = zeaEngine.Registry.constructClass(childActionjson.type);
      if (childAction) {
    ...
    for (const key in j.outputs) {
      ...
      // const output = Registry.constructClass(outputjson.type);
      // if (output) {
...
class StateMachine extends zeaEngine.BaseItem {
   ...
   */
  getActiveStateName() {
    return zeaEngine.Registry.getBlueprintName(this.__currentState)
  }
  ...
  fromJSON(j, context, flags) {
    ...
    for (const key in j.states) {
      ...
      const state = zeaEngine.Registry.constructClass(statejson.type);
      if (state) {
...
zeaEngine.Registry.register('StateMachine', StateMachine);
...
class SwitchState extends StateAction {
  ...
  constructor() {
    super();
    this.__targetStateParam = this.addParameter(new zeaEngine.StringParameter('TargetState', ''));
  }
...
zeaEngine.Registry.register('SwitchState', SwitchState);
...
class SetCameraPositionAndTarget extends StateAction {
  ...
  constructor() {
    ...
    this.addParameter(new zeaEngine.TreeItemParameter('Camera', (treeItem) => treeItem instanceof zeaEngine.Camera));
    ...
    this.addParameter(new zeaEngine.NumberParameter('UpdateFrequency', 100));
  }
  ...
  activate() {
    ...
    if (interpTime > 0.0) {
      ...
      const timerCallback = () => {
        ...
        if (step < steps) {
          const t = step / steps;
          const smooth_t = zeaEngine.MathFunctions.smoothStep(0.0, 1.0, t);
          ...
          const idealDist = zeaEngine.MathFunctions.lerp(distNow, distEnd, delta);
...
zeaEngine.Registry.register('SetCameraPositionAndTarget', SetCameraPositionAndTarget);
...
class SetParameterValue extends StateAction {
  ...
  constructor() {
    super();
    this.__interpTimeParam = this.addParameter(new zeaEngine.NumberParameter('InterpTime', 1.0));

    // Note: if the update frequency here can be faster than the renderer, it means each
    // rendered frame there is an updated values. This keeps movement smooth.
    // This is very apparent when moving the camera, while changing other values. If the 2 changes
    // are slower than rendering, then we see juddering.
    this.__updateFrequencyParam = this.addParameter(new zeaEngine.NumberParameter('UpdateFrequency', 100));
  }
    ...
    if (this.__outParam) {
      ...
      if (interpTime > 0.0) {
        ...
        const timerCallback = () => {
          ...
          if (step < steps) {
            const t = step / steps;
            const smoothT = zeaEngine.MathFunctions.smoothStep(0.0, 1.0, t);
            const newVal = zeaEngine.MathFunctions.lerp(paramValueStart, paramValueEnd, smoothT);
  ...
  toJSON(context, flags) {
    ...
    if (this.__valueParam) {
      j.valueParamType = zeaEngine.Registry.getClassName(this.__valueParam);
    }
    return j
  }
  ...
  fromJSON(j, context, flags) {
    if (j.valueParamType) {
      const param = zeaEngine.Registry.constructClass(j.valueParamType, 'Value');
...
zeaEngine.Registry.register('SetParameterValue', SetParameterValue);
...
zeaEngine.Registry.register('GeomClicked', GeomClicked);
...
zeaEngine.Registry.register('KeyPressedEvent', KeyPressedEvent);
...
zeaEngine.Registry.register('TimedWait', TimedWait);

exports.GeomClicked = GeomClicked;
exports.KeyPressedEvent = KeyPressedEvent;
exports.SetCameraPositionAndTarget = SetCameraPositionAndTarget;
