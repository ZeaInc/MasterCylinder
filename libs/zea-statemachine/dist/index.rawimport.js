import { Registry, ParameterOwner, BaseItem, StringParameter, TreeItemParameter, Camera, Vec3Parameter, NumberParameter, MathFunctions } from '../../zea-engine/dist/index.esm.js';
...
class State {
   ...
   */
  constructor(name) {
    this.__name = name ? name : Registry.getBlueprintName(this);
  ...
  toJSON(context, flags) {
    const j = {
      name: this.__name,
      type: Registry.getBlueprintName(this),
    };
  ...
  fromJSON(j, context, flags) {
    ...
    for (const stateEventJson of j.stateEvents) {
      const stateEvent = Registry.constructClass(stateEventJson.type);
      stateEvent.fromJSON(stateEventJson, context);
      this.addStateEvent(stateEvent);
    }
    for (const activationActionJson of j.activationActions) {
      const activationAction = Registry.constructClass(activationActionJson.type);
    ...
    for (const deactivationActionJson of j.deactivationActions) {
      const deactivationAction = Registry.constructClass(deactivationActionJson.type);
...
class StateAction extends ParameterOwner {
   ...
   */
  activate() {
    console.warn('activate must be implmented by each action. this:' + Registry.getBlueprintName(this));
  }
  ...
  fromJSON(j, context, flags) {
    ...
    for (const childActionjson of j.childActions) {
      const childAction = Registry.constructClass(childActionjson.type);
      if (childAction) {
...
Registry.register('SwitchState', SwitchState);
...
class SetParameterValue extends StateAction {
  ...
  toJSON(context, flags) {
    ...
    if (this.__valueParam) {
      j.valueParamType = Registry.getClassName(this.__valueParam);
    }
    return j
  }
  ...
  fromJSON(j, context, flags) {
    if (j.valueParamType) {
      const param = Registry.constructClass(j.valueParamType, 'Value');
...
Registry.register('SetParameterValue', SetParameterValue);
...
Registry.register('GeomClicked', GeomClicked);
...
Registry.register('KeyPressedEvent', KeyPressedEvent);
...
Registry.register('TimedWait', TimedWait);
